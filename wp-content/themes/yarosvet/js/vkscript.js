(function() {
  ...
  var group_id = '818546'; // вписать id вашей группы
  var topic_id = '33011617'; // вписать id поста
  var JSONReviewsGeneratorReference = '/wp-content/themes/yarosvet/vkreviews.php'; // вписать путь к файлу d_reviews.php (например https://my-site.ru/d_reviews.php)
  var quantityComments = 5; // количество комментариев для вывода
  ...
  // for IE
  if ('content' in document.querySelector('#vkcomment-template')) {
    commentTemplate = document.querySelector('#vkcomment-template').content;
  } else {
    commentTemplate = document.querySelector('.vkcomment');
  }
  ...
  function createCommentElement(data, i) {
    ...
    element.querySelector('.vkcomment__text-title').textContent = getCommentText(data, i);
    element.querySelector('.vkcomment__like-number').textContent = data.items[i].likes.count;
    element.querySelector('.vkcomment__comment-date').href = VK_LINK + 'topic-' + group_id + '_' + topic_id + '?post=' + data.items[i].id;
    ...
    element.querySelector('.vkcomment__logo-image').src = sourcePhoto;
    element.querySelector('.vkcomment__logo').href = sourceIDLink;
    element.querySelector('.vkcomment__online').textContent = stateOnline;
    element.querySelector('.vkcomment__group-title').textContent = nameTitle;
    element.querySelector('.vkcomment__group-title').href = sourceIDLink;
    element.querySelector('.vkcomment__comment-date').textContent = commentDate;
    element.querySelector('.vkcomment__image').src = sourceSticker;
